...
│  ┌─────────────────────┐          ┌─────────────────────────────────────┐   │
│  │   CLI 入口           │          │   Claude Code                       │   │
│  │  freeride ask "..."  │          │  ANTHROPIC_BASE_URL=localhost:8765 │   │
│  │  POST /ask           │          │  POST /v1/messages                  │   │
│  └──────────┬──────────┘          └──────────────────┬──────────────────┘   │
│             │                                        │                       │
│             │         CLI 与 Gateway 并行关系          │                       │
│             └────────────────┬───────────────────────┘                       │
...
### 作为 Claude Code Gateway 使用（推荐）

FreeRide 提供了 **Anthropic-compatible API**，可以被 Claude Code 直接调用：

```bash
# 1. 启动 Native Host
python3 native/native_host.py

# 2. 确保 Chrome 扩展已加载且豆包页面已打开

# 3. 配置 Claude Code 使用 FreeRide Gateway
export ANTHROPIC_BASE_URL=http://localhost:8765
export ANTHROPIC_API_KEY=freeride
...
### v0.4.0
- 新增: Anthropic-compatible API 支持 (`/v1/messages`, `/v1/models`)
- 新增: Claude Code 可通过 `ANTHROPIC_BASE_URL` 直接使用 FreeRide
- 新增: 模型映射规则 (sonnet/opus → think, haiku → quick)
- 优化: 简化架构，无需额外的 Gateway 服务
